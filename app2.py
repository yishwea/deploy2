...
def prediction_disease(input_data):
    ...
    # Now you can use the model to predict probabilities
    predicted_probabilities = load_model.predict_proba(input_data_reshaped)
    ...
    predicted_probabilities = predicted_probabilities.toarray()

    # Create a DataFrame with predicted probabilities
    df = pd.DataFrame(predicted_probabilities, columns=column_names)

    st.header("Predicted Probabilities:")
    st.dataframe(df)
    # st.write("Predicted Probabilities:")
    # for i, probability in enumerate(predicted_probabilities[0]):
    #     st.write(f"{column_names[i]} : {probability:.4f}")
